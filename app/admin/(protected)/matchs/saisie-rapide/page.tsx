...
export default function SaisieRapidePage() {
  ...
  async function loadMatches() {
    ...
    if (!error && data) setMatches(data as unknown as Match[]);
    setLoading(false);
  }
  ...
  async function updateMatch(matchId: string, patch: Record<string, unknown>, action: string) {
    ...
    if (!error) await logAction(matchId, action, patch);
    setSavingId(null);
  }
  ...
  if (matches.length === 0) {
    return <p style={{ padding: 16 }}>Aucun match à venir ou en cours pour l'instant.</p>;
  }
